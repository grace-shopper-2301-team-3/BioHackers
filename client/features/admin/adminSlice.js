...
export const fetchPaymentOptions = createAsyncThunk('paymentOptions/fetchPaymentOptions', async () => {
    const response = await axios.get(`/payment-options`)
    return response.data
})

// Thunk for adding a new payment option
export const addPaymentOption = createAsyncThunk('paymentOptions/addPaymentOption', async (paymentOptionData) => {
    const response = await axios.post(`/payment-options`, paymentOptionData)
    return response.data
})

// Thunk for editing an existing payment option
export const editPaymentOption = createAsyncThunk('paymentOptions/editPaymentOption', async ({ id, paymentOptionData }) => {
    const response = await axios.put(`/payment-options/${id}`, paymentOptionData)
    return response.data
})

// Thunk for removing an existing payment option
export const removePaymentOption = createAsyncThunk('paymentOptions/removePaymentOption', async (id) => {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(editPaymentOption.fulfilled, (state, action) => {
                const index = state.paymentOptions.findIndex((p) => p.id === action.payload.id)
                if (index !== -1) {
                    state.paymentOptions[index] = action.payload
                }
            })
            .addCase(removePaymentOption.fulfilled, (state, action) => {
                state.paymentOptions = state.paymentOptions.filter((p) => p.id !== action.payload)
            })
    }
})
